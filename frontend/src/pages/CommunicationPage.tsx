...
import { FileText, LockKeyhole, MessageSquareReply, MessagesSquare, Send, ShieldCheck } from 'lucide-react'
import { api } from '../services/api'
import type { CommunicationMessage, CommunicationThread, Company, DataAccessLog } from '../types'
...
const endpoints = {
  ...
  company: {
    ...
    messages: (id: string) => `/company-portal/communication/threads/${id}/messages`
  }
}
...
export function CommunicationPage({ mode }: Props) {
  ...
  const selected = useMemo(() => threads.find((thread) => thread.id === selectedId), [selectedId, threads])
  const isSine = mode === 'sine'
  const apiSet = endpoints[mode]

  function loadThreads() {
    api.get<CommunicationThread[]>(apiSet.threads)
      .then(({ data }) => {
        setThreads(data)
  ...
  useEffect(() => {
    ...
    api.get<CommunicationMessage[]>(apiSet.messages(selectedId))
  ...
  async function sendMessage(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      loadThreads()
      const { data } = await api.get<CommunicationMessage[]>(apiSet.messages(selectedId))
      setMessages(data)
    } catch (err: any) {
  ...
  async function createThread(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(err.response?.data?.detail ?? 'Não foi possível criar a conversa.')
    }
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-950">{isSine ? 'Comunicação com empresas' : 'Comunicação com o SINE'}</h1>
          <p className="mt-1 text-sm text-slate-600">Canal oficial, auditável e ligado a vagas, encaminhamentos e currículos quando aplicável.</p>
        </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[360px_1fr]">
        <aside className="space-y-4">
          ...
          <div className="rounded-md border border-slate-200 bg-white">
            <div className="border-b border-slate-100 px-4 py-3 text-sm font-bold text-slate-950">Conversas</div>
            <div className="max-h-[560px] overflow-auto">
              {threads.map((thread) => (
                <button key={thread.id} type="button" onClick={() => setSelectedId(thread.id)} className={`w-full border-b border-slate-100 p-4 text-left hover:bg-emerald-50/60 ${selectedId === thread.id ? 'bg-emerald-50' : 'bg-white'}`}>
              ...
              {threads.length === 0 && <div className="p-4 text-sm text-slate-500">Nenhuma conversa registrada ainda.</div>}
            </div>
          </div>
        </aside>

        <section className="rounded-md border border-slate-200 bg-white">
          {!selected && <div className="p-6 text-sm text-slate-500">Selecione ou crie uma conversa.</div>}
          {selected && (
              ...
              <div className="border-b border-slate-100 p-5">
                ...
                {(selected.worker_name || selected.resume_filename) && (
                  <div className="mt-3 grid gap-2 rounded-md border border-amber-100 bg-amber-50 p-3 text-sm text-amber-950 md:grid-cols-2">
                    <div><FileText size={15} className="mr-1 inline" /> {selected.worker_name ?? 'Candidato vinculado'}</div>
                    <div>{selected.resume_filename ?? 'Currículo vinculado ao encaminhamento'}</div>
                  </div>
                )}
              </div>
              <div className="flex-1 space-y-3 overflow-auto bg-slate-50 p-5">
                {messages.map((item) => (
                  <div key={item.id} className={`max-w-[820px] rounded-md border p-3 ${item.sender_role === 'company' ? 'ml-auto border-emerald-100 bg-emerald-50' : 'border-slate-200 bg-white'}`}>
                    <div className="mb-1 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500"><span>{item.sender_role === 'company' ? 'Empresa' : 'SINE'} · {item.sender_name ?? 'Sistema'}</span><span>{formatDate(item.created_at)}</span></div>
                    <div className="whitespace-pre-wrap text-sm leading-6 text-slate-800">{item.body}</div>
                    {item.message_type !== 'message' && <div className="mt-2 inline-flex items-center gap-1 rounded-full bg-white px-2 py-1 text-xs font-semibold text-emerald-800"><LockKeyhole size={13} /> {item.message_type}</div>}
                  </div>
                ))}
              </div>
              <form onSubmit={sendMessage} className="border-t border-slate-100 p-4">
                <label className="block text-sm font-medium text-slate-700">Responder<textarea required value={body} onChange={(event) => setBody(event.target.value)} rows={3} className="mt-1 w-full rounded-md border border-slate-200 px-3 py-2" /></label>
                <button disabled={!body.trim()} className="tenant-button mt-3 inline-flex items-center gap-2 rounded-md px-4 py-2 text-sm font-semibold disabled:opacity-50"><MessageSquareReply size={16} /> Enviar resposta</button>
              </form>
            </div>
          )}
        </section>
...
export function AuditLgpdPage() {
  ...
  useEffect(() => {
    api.get<DataAccessLog[]>('/audit/data-access').then(({ data }) => setItems(data)).catch(() => setError('Não foi possível carregar a auditoria LGPD.'))
  }, [])
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">Acessos a currículos, candidatos e contextos enviados para empresas.</p>
        </div>
      ...
      <div className="overflow-hidden rounded-md border border-slate-200 bg-white">
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-50 text-slate-600">
            <tr><th className="px-4 py-3">Quando</th><th className="px-4 py-3">Quem acessou</th><th className="px-4 py-3">Candidato/Currículo</th><th className="px-4 py-3">Finalidade</th><th className="px-4 py-3">IP</th></tr>
          </thead>
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-t border-slate-100">
                <td className="px-4 py-3 text-slate-600">{formatDate(item.created_at)}</td>
                <td className="px-4 py-3 font-medium text-slate-900">{item.accessed_by_name ?? 'Sistema'}</td>
                <td className="px-4 py-3 text-slate-600">{item.worker_name ?? 'Candidato'}<div className="text-xs text-slate-500">{item.resume_filename ?? 'Sem PDF'}</div></td>
                <td className="px-4 py-3 text-slate-600">{item.reason ?? item.action}<div className="text-xs text-slate-500">{item.action}</div></td>
                <td className="px-4 py-3 text-slate-600">{item.ip_address ?? '-'}</td>
              </tr>
            ))}
            {items.length === 0 && <tr><td className="px-4 py-6 text-slate-500" colSpan={5}>Nenhum acesso auditado ainda.</td></tr>}
          </tbody>
        </table>
      </div>
      <Link to="/comunicacao" className="inline-flex items-center gap-2 text-sm font-semibold text-emerald-800"><MessagesSquare size={16} /> Ver comunicação oficial</Link>
    </div>
  )
}
