...
class Tenant(UUIDMixin, TimestampMixin, Base):
    ...
    primary_color: Mapped[str] = mapped_column(String(20), default="#14532d", nullable=False)
    secondary_color: Mapped[str] = mapped_column(String(20), default="#0f766e", nullable=False)
    accent_color: Mapped[str] = mapped_column(String(20), default="#f59e0b", nullable=False)
    footer_text: Mapped[str] = mapped_column(String(255), default="Prefeitura Municipal de Jacarezinho", nullable=False)
...
class Worker(UUIDMixin, TimestampMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("tenant_id", "cpf", name="uq_worker_tenant_cpf"),)
...
class Resume(UUIDMixin, TimestampMixin, Base):
    ...
    status: Mapped[str] = mapped_column(String(40), default="pendente_analise", nullable=False)
...
class Job(UUIDMixin, TimestampMixin, Base):
    ...
    modality: Mapped[str] = mapped_column(String(30), default="presencial", nullable=False)
...
class Referral(UUIDMixin, TimestampMixin, Base):
    ...
    referred_by_user_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("users.id"), index=True, nullable=False)
    status: Mapped[str] = mapped_column(String(60), default="encaminhado", nullable=False)
...
class Interview(UUIDMixin, TimestampMixin, Base):
    ...
    tenant_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("tenants.id"), index=True, nullable=False)
    referral_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("referrals.id"), index=True, nullable=False)
...
class DataAccessLog(UUIDMixin, Base):
    ...
    tenant_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("tenants.id"), index=True, nullable=False)
    accessed_by_user_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("users.id"), index=True)
