...
class Company(UUIDMixin, TimestampMixin, Base):
    ...
    responsible_position: Mapped[str | None] = mapped_column(String(120))
    responsible_email: Mapped[str | None] = mapped_column(String(255))
    responsible_phone: Mapped[str | None] = mapped_column(String(30))
    ...
    cnae: Mapped[str | None] = mapped_column(String(40))
    status: Mapped[str] = mapped_column(String(60), default="ativa", index=True, nullable=False)
    ...
    blocked_by_user_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("users.id"), index=True)
